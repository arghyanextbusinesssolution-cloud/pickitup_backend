...
export class AdminRepository {
    async getAnalytics() {
        const userCount = await prisma.user.count({ where: { role: 'SHIPPER' } });
    ...
    async getUserByIdWithMetrics(id: string) {
    ...
    async getCarrierByIdWithMetrics(id: string) {
    ...
    async getLiveParcels() {
        ...
        const getHub = (address: string) => {
            if (!address) return 'LDN';
            for (const city in cityToHub) {
                if (address.toLowerCase().includes(city.toLowerCase())) return cityToHub[city];
            }
